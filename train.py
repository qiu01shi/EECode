...
if __name__ == "__main__":
    ...
    model = Net(
        device=device,
        trigger_size=len(all_triggers), # 2 * len(TRIGGER) + 2
        entity_size=len(all_entities),  # 2 * len(ENTITIES) + 2
        all_postags=len(all_postags),   # len(POSTAGS) + 2
        argument_size=len(all_arguments) # len(ARGUMENTS) + 2
    )
    if device == 'cuda':
        model = model.cuda()
