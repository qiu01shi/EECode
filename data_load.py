...
tokenizer = BertTokenizer.from_pretrained('data/bert-base-uncased-vocab.txt', do_lower_case=False, never_split=(PAD, CLS, SEP, UNK))
...
class ACE2005Dataset(data.Dataset):
    def __init__(self, fpath):
        ...
        with open(fpath, 'r') as f:
            ...
            for item in data:
                words = item['words']
                # append BIO tagged entity_type into entities
                entities = [[NONE] for _ in range(len(words))]
                # append BIO tagged trigger into triggers
                triggers = [NONE] * len(words)
                postags = item['pos-tags']
                arguments = {
                    # entity-mention 中 (start,end,enyity)
                    'candidates': [
                        # ex. (5, 6, "entity_type_str"), ...
                    ],
                    # key 是 (trigger['start'],trigger['end'],event_type)
                    # value 是 argument 中 (start, end, argument2idx[role])
                    'events': {
                        # ex. (1, 3, "trigger_type_str"): [(5, 6, "argument_role_idx"), ...]
                    },
                }

                # 对 entity_type 进行 BIO 处理；
                # B、I标记的数目等于 text 的长度
                ...
                # 对 event_type 进行 BIO 处理
                # trigger 相应的 event_type 大多为一个单词
    ...
    # 对 trigger 列表进行权重处理，里面包含 trigger 设置为 5，不包含设置为 1。
        samples_weight = []
